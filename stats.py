...
df = pd.read_csv('results.txt', sep='_', names=['label', 'p1', 's1', 'p2', 's2', 'p3', 's3', 'p4', 's4', 'p5', 's5'])
...
df['accuracy5'] = (df['label'] == df['p1']) | (df['label'] == df['p2']) | (df['label'] == df['p3']) | (df['label'] == df['p4']) | (df['label'] == df['p5'])
print('Top 5 分类正确率：%f' % (np.sum(df['accuracy5']) / len(df)))
